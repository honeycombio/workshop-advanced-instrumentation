...
func getYear(ctx context.Context) (int, error) {
	...
	logger.DebugContext(ctx, "Got year", "year", data.Year)

	return data.Year, nil
}

func getName(ctx context.Context, year int) string {
	names := namesByYear[year]
	rnd := rand.Intn(len(names))
	logger.DebugContext(ctx, "Getting name...", "for year", year, "name", names[rnd])
	return names[rnd]
}
