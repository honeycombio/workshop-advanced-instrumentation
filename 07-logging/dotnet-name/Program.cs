...
var logger = app.Services.GetRequiredService<ILoggerFactory>().CreateLogger("dotnet-name");
...
app.MapGet("/name", async (HttpContext ctx, IHttpClientFactory clientFactory) =>
{
    ...
    logger.LogDebug("Got year {Year}", year.Value);
...
string GetName(int year)
{
    ...
    logger.LogDebug("Getting name... For year {Year}, name {Name}", year, name);
    return name;
}
